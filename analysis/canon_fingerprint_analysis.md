...
date: 2026-02-25
status: superseded
superseded_by: Halfnote_最终审判_经文执行模型归因.md
...
> **SUPERSEDED**: This is an early Round 1 analysis (Feb 25). Its conclusion — that the scriptures were most likely generated by Claude Sonnet 4/4.5 — was overturned by subsequent rounds of investigation (R2: 11-model active probes; R3: OpenRouter API usage data). The definitive conclusion is in [`Halfnote_最终審判_经文执行模型归因.md`](Halfnote_最终审判_经文执行模型归因.md): **Kimi K2.5 is the primary suspect** (#1), with Claude downgraded to #4. Key reasons this early analysis was wrong: (1) em dash comparison used Q&A specimens vs religious text — a genre confound; (2) Chinese models behave very differently when prompted to generate scripture vs answering questions; (3) OpenRouter data showed Kimi K2.5 consumed 985B tokens on the platform; (4) 48,507 words of canon contain zero Claude cognitive hedging patterns. This document is preserved for methodological transparency.

# Crustafarianism 经文模型指纹分析

## Executive Summary

> **Note: The conclusions below were superseded. See header.**

对 1,073 条 Crustafarian 经文进行 7 维度模型指纹分析后，结论如下：

1. ~~**经文最可能来自 Claude 家族**，具体为 **Sonnet 4.5 或 Sonnet 4** 范围内的模型~~ → **Superseded: Kimi K2.5 is the primary suspect.**
...
5. ~~**中国蒸馏模型全部排除** — Kimi K-2.5、GLM-5、MiniMax M-2.5 的 em dash 和 emphasis 指纹与 Canon 不兼容~~ → **Superseded: Genre confound invalidated these exclusions.**
